...
export default function Egresos() {
  ...
  const infoComponent = {
    NameComponent: "Egresos",
    MinusName: "egresos",
    SingularName: "egreso",
    Descripcion: "Aqui puedes gestionar las egresos de tu turno",
    Url: "egresos/",
  };
  useEffect(() => {
    ObtenerLista();
  ...
  const ObtenerLista = async () => {
    ...
    try {
      const result = await axios.post(
        apiUrl + `${infoComponent.Url}Listar`,
        {
  ...
  const Crear = async (e) => {
    ...
    try {
      ...
      const result = await axios.post(
        apiUrl + `${infoComponent.Url}crearEgreso`,
        Egreso,
        {
          withCredentials: true,
        }
  ...
  const Editar = async (e) => {
    ...
    try {
      ...
      //si no se llena la informacion se sale de esta funcion y no se envia nada al backend
      const result = await axios.post(
        apiUrl + `${infoComponent.Url}Editar`,
        Egreso,
        {
          withCredentials: true,
        }
  ...
  const Eliminar = async () => {
    ...
    try {
      //si no se llena la informacion se sale de esta funcion y no se envia nada al backend
      const result = await axios.post(
        apiUrl + `${infoComponent.Url}Eliminar`,
        Egreso,
        {
          withCredentials: true,
        }
          ...
          >
            ...
            <div className="w-[60vh]  ">
              <form onSubmit={GuardarDatos} className="  flex flex-col">
                <p className="text-lg">Nombre del egreso:</p>
                <input
