...
export default function Departamentos() {
  ...
  const infoComponent = {
    NameComponent: "Pacientes",
    MinusName: "pacientes",
    SingularName: "paciente",
    Descripcion: "Aqui puedes gestionar los pacientes",
  };
  useEffect(() => {
    ObtenerLista();
  }, []);
  ...
  const ObtenerLista = async () => {
    setLoader(true);
    try {
      const response = await fetch(apiUrl + "paciente/listarPacientes", {
        method: "GET",
  ...
  const EditarPaciente = async (e) => {
    ...
    try {
      ...
      //si no se llena la informacion se sale de esta funcion y no se envia nada al backend
      const result = await axios.post(
        apiUrl + "paciente/editarPaciente",
        Paciente,
        {
          withCredentials: true,
        }
  ...
  const TABLE_HEAD = ["N", "Paciente", "Editar", "Eliminar"];
  ...
  const ConfirmarEliminar = (id) => {
    SetPaciente({ ...Paciente, pacienteID: id });
  ...
  const Eliminar = async () => {
    ...
    try {
      //si no se llena la informacion se sale de esta funcion y no se envia nada al backend
      const result = await axios.post(
        apiUrl + "paciente/eliminarPaciente",
        Paciente,
        {
          withCredentials: true,
        }
  ...
  const ObtenerListaBusqueda = async () => {
    ...
    try {
      const result = await axios.post(
        apiUrl + `paciente/listarPacientesBusqueda`,
        ...
        <div className="bg-blue-100 dark:bg-black/40  w-full rounded-2xl p-10 flex flex-row gap-6 ">
          ...
          >
            ...
            <div className="w-[60vh]  ">
              <p className="text-lg">Nombre del paciente:</p>

              <form onSubmit={GuardarDatos} className="  flex flex-col">
                <input
                  placeholder="Ejemplo: Jhon Herrera"
          ...
          <div className=" w-full">
            <div className="rounded-2xl p-10  bg-white dark:bg-black/40 gap-3  flex flex-col text-black">
              ...
              <div className="rounded-3xl ">
                {ListaPacientes && ListaPacientes.length != 0 ? (
                  <table className="w-full min-w-max table-auto text-left rounded-2xl">
                    ...
                    <tbody>
                      {ListaPacientes.map(({ pacienteid, paciente }, index) => {
                        const isLast = index === ListaPacientes.length - 1;
                          ...
                          >
                            ...
                            <td className={classes}>
                              <button
                                ...
                                onClick={() =>
                                  ClickEditar(pacienteid, paciente)
                                }
                              >
                            ...
                            <td className={classes}>
                              <button
                                ...
                                onClick={() => ConfirmarEliminar(pacienteid)}
                              >
