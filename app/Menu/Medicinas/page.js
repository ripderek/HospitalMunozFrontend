...
export default function Medicinas() {
  ...
  const infoComponent = {
    NameComponent: "Medicinas",
    MinusName: "medicinas",
    SingularName: "medicina",
    Descripcion: "Aqui puedes gestionar las medicinas",
    Url: "medicina/",
  };
  useEffect(() => {
    ObtenerLista();
  ...
  const ObtenerLista = async () => {
    ...
    try {
      const response = await fetch(
        apiUrl + `${infoComponent.Url}listarMedicina`,
        {
          method: "GET",
  ...
  const HandleChange = (e) => {
    SetMedicina({ ...Medicina, [e.target.name]: e.target.value });
  };
  ...
  const VerficarInformacion = () => {
    ...
    if (!isValidDecimal(Medicina.valor)) {
  ...
  const Crear = async (e) => {
    ...
    try {
      ...
      const result = await axios.post(
        apiUrl + `${infoComponent.Url}crearMedicina`,
        Medicina,
        {
  ...
  const Editar = async (e) => {
    ...
    try {
      ...
      //si no se llena la informacion se sale de esta funcion y no se envia nada al backend
      const result = await axios.post(
        apiUrl + `${infoComponent.Url}editarMedicina`,
        Medicina,
        {
          withCredentials: true,
        }
  ...
  const ConfirmarEliminar = (id) => {
    SetMedicina({ ...Medicina, medicinaID: id });
  ...
  const Eliminar = async () => {
    ...
    try {
      //si no se llena la informacion se sale de esta funcion y no se envia nada al backend
      const result = await axios.post(
        apiUrl + `${infoComponent.Url}eliminarMedicina`,
        Medicina,
        {
          withCredentials: true,
        }
      ...
      SetAlerta({
        ...
        Open: true,
        Titulo: "Eliminado",
        Mensaje: `Se ha eliminado la ${infoComponent.SingularName}`,
        Tipo: "Fail",
      });
      ObtenerLista();
      setLoader(false);
  ...
  const ObtenerListaBusqueda = async () => {
    ...
    try {
      const result = await axios.post(
        apiUrl + `${infoComponent.Url}ListarBusqueda`,
          ...
          >
            ...
            <div className="w-[60vh]  ">
              <form onSubmit={GuardarDatos} className="  flex flex-col">
                <p className="text-lg">Nombre del medicamento:</p>
                <input
                  placeholder="Medicina ejemplo: Paracetamol"
                  className="flex-1 focus:outline-none gap-2  p-4 w-full rounded-2xl   bg-white text-black"
                  name="medicina"
                  onChange={HandleChange}
                  value={Medicina.medicina}
                />
                          ...
                          >
                            ...
                            <td className={classes}>
                              <button
                                ...
                                onClick={() => ConfirmarEliminar(medicinaid)}
                              >
